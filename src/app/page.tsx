...
// Esta abordagem remove a necessidade de instalar a biblioteca 'react-icons'
// e corrige os erros de compilação.

const MdMail = (props: React.SVGProps<SVGSVGElement>) => (
  <span className="material-symbols-outlined text-4xl">
    Mail
  </span>

);
const MdArrowForward = (props: React.SVGProps<SVGSVGElement>) => (
  <span className="material-symbols-outlined text-4xl">
    arrow_forward
  </span>
);
const MdMenu = (props: React.SVGProps<SVGSVGElement>) => (
  <span className="material-symbols-outlined text-4xl">
    Menu
  </span>
);
const MdClose = (props: React.SVGProps<SVGSVGElement>) => (
  <span className="material-symbols-outlined text-4xl">
    close
  </span> 
);
const MdSunny = (props: React.SVGProps<SVGSVGElement>) => (
  <span className="material-symbols-outlined text-4xl">
    light_mode
  </span> 
);

const MdNightlight = (props: React.SVGProps<SVGSVGElement>) => (
 <span className="material-symbols-outlined text-4xl">
    dark_mode
  </span>
);

const FaLinkedin = (props: React.SVGProps<SVGSVGElement>) => (
...
const FaGithub = (props: React.SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg"  width="1em" height="1em" viewBox='0 0 48 48' {...props}>
...
const FaInstagram = (props: React.SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg"  width="1em" height="1em" viewBox='0 0 48 48' {...props}>
...
const FaFacebook = (props: React.SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg"  width="1em" height="1em" viewBox='0 0 48 48' {...props}>
...
const SiOrcid = (props: React.SVGProps<SVGSVGElement>) => (
  <span className="material-symbols-outlined text-4xl">
    school
  </span>);

// --- DADOS DE CONTEÚDO (i18n) ---
const content = {
  pt: {
    ...
    footer: { socialCall: 'Me acompanhe nas redes sociais', copyright: `© ${new Date().getFullYear()} Diogo Cunha. Todos os direitos reservados.` },
  },
  en: {
    ...
    footer: { socialCall: 'Follow me on social media', copyright: `© ${new Date().getFullYear()} Diogo Cunha. All rights reserved.` },
  },
};
...
const ProjectCard = ({ title, description, tags, githubUrl, liveUrl, lang }: ProjectCardProps) => (
  <div className="bg-white dark:bg-slate-800/50 rounded-lg p-6 border border-slate-200 dark:border-slate-700 hover:border-violet-500 dark:hover:border-violet-500 transition-all duration-300 flex flex-col h-full shadow-sm hover:shadow-lg">
    ...
    <div className="flex flex-wrap gap-2 mb-4">{tags.map((tag: string) => (<span key={tag} className="bg-violet-100 text-violet-800 dark:bg-slate-700 dark:text-violet-300 text-xs font-medium px-2.5 py-0.5 rounded-full">{tag}</span>))}</div>
    <div className="mt-auto pt-4 border-t border-slate-200 dark:border-slate-700/50 flex items-center gap-4">
      <a href={githubUrl} target="_blank" rel="noopener noreferrer" className="text-slate-600 dark:text-slate-300 hover:text-violet-600 dark:hover:text-violet-400 transition-colors duration-300 flex items-center gap-2 text-sm"><FaGithub width={16} height={16} /> {content[lang].projects.viewOnGithub}</a>
      <a href={liveUrl} target="_blank" rel="noopener noreferrer" className="text-slate-600 dark:text-slate-300 hover:text-violet-600 dark:hover:text-violet-400 transition-colors duration-300 flex items-center gap-2 text-sm"><MdArrowForward width={16} height={16} /> {content[lang].projects.viewDemo}</a>
    </div>
  </div>
);
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    setLanguage(savedLang as 'pt' | 'en');
  }, []);
  ...
  return (
    <div className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-300 font-sans antialiased">
      <header className="fixed top-0 left-0 right-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm z-50 border-b border-slate-200 dark:border-slate-800">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <a href="#" className="text-2xl font-bold text-slate-900 dark:text-white">Diogo Cunha<span className="text-violet-600 dark:text-violet-400">.</span></a>
            <div className="flex items-center gap-2 sm:gap-4">
              <nav className="hidden md:flex items-center gap-8">
                <NavLink href="#sobre">{currentContent.nav.about}</NavLink>
        ...
        {isMenuOpen && (<div className="md:hidden bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800"><nav className="flex flex-col items-center gap-4 py-4"><a href="#sobre" onClick={() => setIsMenuOpen(false)} className="text-slate-600 dark:text-slate-300 hover:text-violet-600 dark:hover:text-violet-400 py-2">{currentContent.nav.about}</a><a href="#projetos" onClick={() => setIsMenuOpen(false)} className="text-slate-600 dark:text-slate-300 hover:text-violet-600 dark:hover:text-violet-400 py-2">{currentContent.nav.projects}</a><a href="#contato" onClick={() => setIsMenuOpen(false)} className="text-slate-600 dark:text-slate-300 hover:text-violet-600 dark:hover:text-violet-400 py-2">{currentContent.nav.contact}</a></nav></div>)}
      </header>
      <main className="pt-16">
        <section id="hero" className="min-h-screen flex items-center bg-grid-slate-200/[0.2] dark:bg-grid-slate-800/[0.2]">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 className="text-4xl md:text-6xl font-extrabold text-slate-900 dark:text-white leading-tight mb-4">{currentContent.hero.title} <br className="hidden md:block" /><span className="text-violet-600 dark:text-violet-400">{currentContent.hero.subtitle}</span></h1>
            <p className="max-w-2xl mx-auto text-lg md:text-xl text-slate-600 dark:text-slate-400 mb-8">{currentContent.hero.description}</p>
            ...
            <div className="grid grid-cols-11 items-center justify-around gap-8 md:gap-12 ">
              ...
              <div className="md:col-span-7">
                ...
                <div className="flex items-center gap-6">
                  <SocialLink href={professionalLinks.linkedin} icon={FaLinkedin} label="LinkedIn" />
                  <SocialLink href={professionalLinks.github} icon={FaGithub} label="GitHub" />
                  <SocialLink href={professionalLinks.orcid} icon={SiOrcid} label="ORCID" />
                  <SocialLink href={professionalLinks.figma} icon={SiFigma} label="Figma" />
                </div>
              </div>
            </div>
          </div>
        </section>
        <section id="tecnologias" className="py-20 md:py-32 bg-slate-50 dark:bg-slate-900/70">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12"><h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white">{currentContent.tech.title}</h2></div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {Object.entries(technologies).map(([key, value]) => (
                        <div key={key} className="bg-white dark:bg-slate-800 p-6 rounded-lg border border-slate-200 dark:border-slate-700 text-center">
                            <img src={value.icon} alt={`${key} icon`} className="mx-auto h-12 w-12 mb-4" />
                            <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-2">{currentContent.tech[key as keyof typeof currentContent.tech]}</h3>
                            <p className="text-slate-600 dark:text-slate-400 mb-4 text-sm">{currentContent.tech[`${key}Desc` as keyof typeof currentContent.tech]}</p>
                            <div className="flex flex-wrap justify-center gap-2">{value.items.map(item => (<span key={item} className="bg-violet-100 text-violet-800 dark:bg-slate-700 dark:text-violet-300 text-xs font-medium px-2.5 py-0.5 rounded-full">{item}</span>))}</div>
                        </div>
                    ))}
                </div>
            </div>
        ...
        <section id="projetos" className="py-20 md:py-32">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white">{currentContent.projects.title}</h2>
              <p className="max-w-2xl mx-auto mt-4 text-slate-600 dark:text-slate-400">{currentContent.projects.description}</p>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">{portfolioProjects.map((project) => (<ProjectCard key={project.title} {...project} lang={language} />))}</div>
          </div>
        </section>
        <section id="contato" className="py-20 md:py-32 bg-slate-50 dark:bg-slate-900/70">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white">{currentContent.contact.title}</h2>
      ...
      <footer className="border-t border-slate-200 dark:border-slate-800">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10 text-center text-slate-500 dark:text-slate-500">
            <h3 className="text-slate-700 dark:text-slate-300 font-bold mb-4">{currentContent.footer.socialCall}</h3>
            <div className="flex justify-center items-center gap-6 mb-6">
                <a href={professionalLinks.linkedin} aria-label="LinkedIn" target="_blank" rel="noopener noreferrer" className="text-[#0077B5] hover:opacity-80 transition-opacity"><FaLinkedin width={48} height={48} /></a>
                <a href={professionalLinks.github} aria-label="GitHub" target="_blank" rel="noopener noreferrer" className="text-slate-800 dark:text-white hover:opacity-80 transition-opacity"><FaGithub width={48} height={48} /></a>
                <a href={professionalLinks.instagram} aria-label="Instagram" target="_blank" rel="noopener noreferrer" className="text-[#E4405F] hover:opacity-80 transition-opacity"><FaInstagram width={48} height={48} /></a>
                <a href={professionalLinks.facebook} aria-label="Facebook" target="_blank" rel="noopener noreferrer" className="text-[#1877F2] hover:opacity-80 transition-opacity"><FaFacebook width={48} height={48} /></a>
                <a href={professionalLinks.figma} aria-label="Figma" target="_blank" rel="noopener noreferrer" className="text-slate-800 dark:text-white hover:opacity-80 transition-opacity"><SiFigma width={48} height={48} /></a>
            </div>
